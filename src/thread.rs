...
mod tests {
    ...
    #[tokio::test]
    async fn test_load_exposes_modes_for_default_session() -> anyhow::Result<()> {
        ...
        let modes = load
            .modes
            .expect("default sessions should expose modes in load responses");
        assert!(
            !modes.available_modes.is_empty(),
            "default sessions should advertise at least one available mode"
        );
    ...
    #[tokio::test]
    async fn test_load_exposes_modes_for_trusted_session() -> anyhow::Result<()> {
        let session_id = SessionId::new("test");
        let client = Arc::new(StubClient::new());
        let session_client =
            SessionClient::with_client(session_id, client, Arc::default());
        ...
        let modes = load
            .modes
            .expect("trusted sessions should expose modes in load responses");
        assert_eq!(modes.current_mode_id.0.as_ref(), "auto");
        assert!(
            !modes.available_modes.is_empty(),
            "trusted sessions should advertise at least one available mode"
        );
        ...
        config
            ...
            .set(SandboxPolicy::ReadOnly {
                access: ReadOnlyAccess::Restricted {
                    ...
                    readable_roots: vec!["/tmp".try_into().expect("/tmp is absolute")],
                },
